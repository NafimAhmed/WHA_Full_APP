...
class FSignUpController extends GetxController {
  ...
  final SignupPharmacyService signupService = Get.find<SignupPharmacyService>();
  ...
  void submitSignUpForm() async {
    ...
    if (isValid) {
      ...
      bool feedback = await signupService.signupPharmacy(
          name: name,
          address: address,
          image: strImage,
          phone: phone,
  ...
  getAddress() async {
    ...
    for (var element in detail.result.addressComponents) {
      ...
      address = addressValueController.text;
    }
  }
}
