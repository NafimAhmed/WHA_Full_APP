...
class FAllAppointmentsController extends GetxController{
  final AppState appState = Get.find<AppState>();
  final AppointmentProvider appointmentProvider = Get.find<AppointmentProvider>();
  ...
  cancelAppointment({required Appointment appointment}) async {
    isCancelling = true;
    liveAppointments =
        (liveAppointments as List<Appointment>).where((element) => element.id !=
            appointment.id).toList();
